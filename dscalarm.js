...
var sharedLocation = nconf.get('dscalarm:sharedLocation');
var appid = nconf.get('dscalarm:appid');
var accessToken = nconf.get('dscalarm:accessToken');
...
// Used to send a keystroke to the alarm panel
app.get("/api/sendKeyPress/:key", function (req, res) {

    var keyPressed = req.params.key;
...
app.get('/config/:host', function (req, res) {
    var parts = req.params.host.split(":");
    if(parts != "null"){
        nconf.set('dscalarm:alarmpassword', parts[0]);
        nconf.set('dscalarm:sharedLocation', parts[1]);
        nconf.set('dscalarm:appid', parts[2]);
        nconf.set('dscalarm:accessToken', parts[3]);
        nconf.save(function (err) {
            if (err) {
...
//reload variables from the config file
function reloadConfig() {

    alarmPassword = nconf.get('dscalarm:alarmpassword');
    baudRate = nconf.get('dscalarm:baudRate');
    sharedLocation = nconf.get('dscalarm:sharedLocation');
    appid = nconf.get('dscalarm:appid');
    accessToken = nconf.get('dscalarm:accessToken');
    ...
    if (cmdfullstr.length >= 3){
        ...
        if (cmd == "609") {
            ...
            if (cmdfullstr.indexOf("Door Chime") >= 0) {
                ...
                var lcdDisplay = cmdfullstr.replace(/\//g, ' ');
	        var msg = ("SY-" + encodeURIComponent(lcdDisplay.substring(0,40)));
                sendSmartThingMsg(msg);
 	    }
        }
        else if (cmd == "658") {
            ...
            if (cmd == "903") {
                var msg = ("SY-" + cmdfullstr.substr(0, 5));
                sendSmartThingMsg(msg);
            }
        }
    }
...
function sendSmartThingMsg(command) {
    var pathURL = '/api/smartapps/installations/' + appid + '/dscalarm/' + command + '?access_token=' + accessToken;
